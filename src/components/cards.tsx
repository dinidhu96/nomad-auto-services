...
export function StatCard({ icon, label, value }: { icon: string; label: string; value: string }) {
  return (
    <div className="flex items-center gap-4 border-white/10 p-4 lg:border-r last:border-r-0">
...
export function TestimonialCard({ name, quote }: { name: string; quote: string }) {
  return (
    <article className="flex items-center gap-4 border-white/10 px-4 py-5 lg:border-r last:border-r-0">
      <div className="grid h-16 w-16 shrink-0 place-items-center rounded-full bg-[#FFC526] text-xl font-black text-[#001240]">
        {name[0]}
      </div>
      <div>
        <p className="text-sm leading-6 text-white">&quot;{quote}&quot;</p>
        <div className="mt-2 flex items-center gap-2 text-sm font-bold text-[#FFC526]">
          - {name} <span className="flex">{Array.from({ length: 5 }).map((_, index) => <Star key={index} className="h-4 w-4 fill-current" />)}</span>
        </div>
      </div>
    </article>
  );
...
export function PricingCard({ plan, highlighted = false }: { plan: PricingPlan; highlighted?: boolean }) {
  return (
    <article className={`glass rounded-2xl p-6 ${highlighted ? "border-[#FFC526]/80 shadow-[0_0_0_1px_rgba(248,176,0,.4)]" : ""}`}>
      <h3 className="text-2xl font-black">{plan.name}</h3>
      <p className="mt-2 min-h-12 text-sm leading-6 text-[#C9D6F5]">{plan.description}</p>
      <div className="mt-6 flex items-end gap-1">
        <span className="text-4xl font-black text-[#FFC526]">{formatMoney(plan.price)}</span>
