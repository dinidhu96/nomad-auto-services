    ...
    <footer className="border-t border-white/10 bg-[#001240] pb-24 lg:pb-0">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 md:grid-cols-2 lg:grid-cols-5 lg:px-6">
        <div className="text-center lg:col-span-2 lg:text-left">
          <Image src="/assets/logo-horizontal-crop.jpeg" alt={business.name} width={320} height={112} className="mx-auto h-16 w-auto lg:mx-0" />
          <p className="mx-auto mt-4 max-w-sm text-sm leading-6 text-[#C9D6F5] lg:mx-0">{business.slogan}</p>
        ...
        <div className="grid gap-5">
          ...
          <div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-xs font-bold text-[#001240] sm:flex sm:flex-wrap">
              {siteContent.paymentMethods.map((method) => (
                <span key={method} className="rounded-md bg-white px-2 py-1 text-center">{method}</span>
              ))}
            </div>
          </div>
        </div>
